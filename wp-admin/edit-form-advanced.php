...
if ( 'post' == $post_type ) {
	...
	'<h3>' . __('SoWink how-to, please learn how the blog and the SoWink newsfeed work together.') . '</h3>' .
	'<p>' . __('Images in the post: To add an image, you have to upload it assigned with the post you\'re editing by clicking the Upload/Insert image icon above the post\'s content text area. Now to actually insert them to the post, you have to edit them individually and click the "Insert into post" button for each. This inserts a HTML snippet into the post content text area (Something with <img>) that places the image in the text wherever the snippet is. If you cut and paste it somewhere else inside the text, it will just show up there.') . '</p>' .
	'<p>' . __('If you have multiple images uploaded to a post, you\'ll see a list of images inside the admin area. Note that the order of this list has nothing to do with the order you have your <img> snippets in your post\'s text. You have to set the order field to 1 next to the image you want to be sent to the SoWink website newsfeeds as the picture in the newsfeed item.') . '</p>' .
	'<p>' . __('') . '</p>' .
	'<p>' . __('Please make our blog\'s newsfeed easily readable on the SoWink website by providing a summary of the post! After you have written your post, scroll down to the area titled Custom Fields, and create a summary. In case you can\'t find the Custom Fields area, you can enable it after clicking "Screen Options" at the top right corner of this page.') . '</p>' .
	'<p>' . __('') . '</p>' .
	'<h3>' . __('WordPress how-to') . '</h3>' .
